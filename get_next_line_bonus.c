...
char	*ft_get_nbalance(char *balance_line)
{
	...
	nbalance = (char *)malloc(sizeof(char) * (ft_strlen(balance_line) - i + 1));
	if (!nbalance)
		return (NULL);
	i++;
	j = 0;
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	balance_line[fd] = ft_read_file(fd, balance_line[fd]);
	if (!balance_line[fd])
		return (NULL);
	line = ft_get2n(balance_line[fd]);
	balance_line[fd] = ft_get_nbalance(balance_line[fd]);
	return (line);
}
